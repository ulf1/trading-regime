...
class MarkovRegimeSwitching(nn.Module):
    ...
    """
    def __init__(self, num_series: int, num_states: int):
        super().__init__()
    ...
    def sort_regimes(self) -> None:
        ...
        ensuring that state 0 consistently represents the "high-mean/bull" regime, 
        state 1 represents "intermediate", and state K-1 represents the "low-mean/bear" regime.
        """
...
def train_mrs_model(
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 50 == 0 or epoch == 0:
            print(f"Epoch {epoch+1:4d} | Total NLL Loss: {loss.item():.4f}")
...
def predict_next_step(
    ...
    """
    Predicts the state probabilities and expected value for the next time step (t + 1).
    
    Args:
        model: The trained MarkovRegimeSwitching model.
        current_prob: Tensor of shape (N, K) representing the filtered probabilities
                      of being in each state at the current time step t.
                      
    Returns:
        next_state_prob: Tensor of shape (N, K) containing predicted state probabilities for t+1.
        expected_y: Tensor of shape (N,) containing expected observation values for t+1.
    """
    model.eval()
    
    # Get valid parameters
    mu, _, trans_mat = model.get_constrained_params()
    
    # 1. Predict Next State Probabilities: P(S_{t+1} | Y_t) = P(S_t | Y_t) * transition_matrix
    # current_prob: (N, K) -> unsqueezed to (N, 1, K)
    # trans_mat: (N, K, K)
    # Result of bmm: (N, 1, K) -> squeezed to (N, K)
    next_state_prob = torch.bmm(current_prob.unsqueeze(1), trans_mat).squeeze(1)
    
    # 2. Predict Next Observation: E[y_{t+1} | Y_t] = Sum_{k} ( P(S_{t+1} = k | Y_t) * mu_k )
    expected_y = torch.sum(next_state_prob * mu, dim=1)
    
    return next_state_prob, expected_y
